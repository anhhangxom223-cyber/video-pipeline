...
class ConfigLoader:
    ...
    def _validate_face_fusion_config_values(cls, data: Dict[str, str]) -> None:
        ...
        if mode not in cls._FACE_FUSION_ALLOWED_MODES:
            raise FaceFusionInputError(
                f"invalid face_fusion_mode: {data['face_fusion_mode']}"
            )

        cls._parse_float_range(
            data["face_fusion_strength"], "face_fusion_strength"
        )
        cls._parse_float_range(
            data["face_fusion_min_confidence"],
            "face_fusion_min_confidence",
        )

        if not isinstance(data["face_fusion_reference_face_path"], str):
            raise FaceFusionInputError(
                "face_fusion_reference_face_path must be a string"
            )
    ...
    def build_face_fusion_config(
        ...
        if isinstance(resolved_config, ResolvedConfig):
            ...
            raise TypeError(
                "resolved_config must be ResolvedConfig or Dict[str, str]"
            )
        ...
        if not reference_face_path:
            raise FaceFusionInputError(
                "face_fusion_reference_face_path must be non-empty"
            )

        return FaceFusionConfig(
            enabled=cls._parse_bool(
                data["face_fusion_enabled"], "face_fusion_enabled"
            ),
            mode=data["face_fusion_mode"].strip().lower(),
            reference_face_path=reference_face_path,
            strength=cls._parse_float_range(
                data["face_fusion_strength"], "face_fusion_strength"
            ),
            min_confidence=cls._parse_float_range(
                data["face_fusion_min_confidence"],
                "face_fusion_min_confidence",
            ),
...
class StageIsolation:
    ...
    def run_stage(stage_name: str, func: Callable, *args, **kwargs) -> Any:
        ...
        try:
            ...
            elapsed = (time.monotonic() - start) * 1000
            logger.error(
                "Stage %s FAILED after %.2fms: %s", stage_name, elapsed, e
            )
            raise
